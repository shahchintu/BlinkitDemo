...
import { ICouponValidation, IProduct } from '../../../core/models';
      ...
      <div class="fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-blinkit-border">
          <div class="flex items-center gap-2">
            ...
            @if ((cartService.cart$ | async)?.itemCount; as count) {
              <span class="text-xs bg-blinkit-green text-white px-2 py-0.5 rounded-full font-bold">{{ count }}</span>
            }
          </div>
          <button
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          @if (cart$ | async; as cart) {
            @if (cart.items.length === 0) {
              ...
              <!-- Delivery nudge -->
              @if (cart.subTotal < 199 && cart.subTotal > 0) {
        ...
        <!-- Footer CTA -->
        @if ((cartService.cart$ | async)?.items?.length) {
...
export class CartSidebarComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.cart$.pipe(
      tap(cart => {
        if (cart.items.length > 0) {
          this.productService
            .getRelatedProducts(cart.items[0].productId, 4)
  ...
  saveForLater(item: CartItemDto): void {
    const wishlist: CartItemDto[] = JSON.parse(localStorage.getItem('wishlist') ?? '[]');
    if (!wishlist.some(w => w.variantId === item.variantId)) {
      wishlist.push(item);
